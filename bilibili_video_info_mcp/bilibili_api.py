...
def _resolve_sessdata(sessdata: Optional[str]) -> Optional[str]:
    if sessdata:
        return sessdata
    return os.getenv("SESSDATA")


def _get_headers(sessdata: Optional[str] = None, allow_missing: bool = False) -> dict:
    headers = DEFAULT_HEADERS.copy()
    resolved = _resolve_sessdata(sessdata)
    if not resolved:
        if allow_missing:
            return headers
        raise ValueError("SESSDATA environment variable is required")
    headers['Cookie'] = f'SESSDATA={resolved}'
    return headers
...
def extract_bvid(url):
    ...
    if 'b23.tv' in url:
        try:
            response = requests.head(url, headers=_get_headers(allow_missing=True), allow_redirects=True)
...
def get_video_basic_info(bvid, sessdata: Optional[str] = None):
    """Gets aid and cid for a given bvid."""
    headers = _get_headers(sessdata=sessdata)
    try:
...
def get_subtitles(aid, cid, sessdata: Optional[str] = None):
...
def get_danmaku(cid, sessdata: Optional[str] = None):
...
def get_comments(aid, sessdata: Optional[str] = None):
